...
function shallowCopy(obj) {
  const newObj = {};
  const copyObj = Object.assign(newObj, obj);
  return copyObj;
}
...
function compareObjects(obj1, obj2) {
  const [arrFirstOjb, arrSecondOjb] = [
    Object.entries(obj1).flat(),
    Object.entries(obj2).flat(),
  ];
  const result = arrFirstOjb.every((element) => arrSecondOjb.includes(element));
...
function sellTickets(queue) {
  ...
  const lastPay = queue.at(-1);
  const sumTickets = queue
    .slice(0, queue.length - 1)
    .reduce((acc, sum) => acc + sum, 0);
  const isSell = sumTickets - lastPay;
